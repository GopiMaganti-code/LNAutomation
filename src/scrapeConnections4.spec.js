...
// Config
// -------------------------
const STORAGE_FILE = process.env.STORAGE_FILE || "connections-state.json"; // persistent full browser state
...
test("LinkedIn login + scrape all connections (UI full scroll only)", async ({}) => {
  ...
  // Launch persistent browser context with saved state
  const context = await chromium.launchPersistentContext("", {
    headless: false,
    storageState: fs.existsSync(STORAGE_FILE) ? STORAGE_FILE : undefined,
  });

  // Reuse first tab (avoid opening 2nd one)
  ...
  if (
    ...
    await context.storageState({ path: "connections-state.json" });
    console.log("💾 Storage state saved to connections-state.json");
  } else {
