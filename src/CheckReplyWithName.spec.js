        ...
        if (await messageButton.isVisible({ timeout: 10000 }).catch(() => false)) {
          ...
          if (replyElements.length > 0) {
            ...
            for (let i = 0; i < replyElements.length; i++) {
              ...
              // Use the same sender and timestamp for all messages in the thread if not found individually
              const localSender = await replyElement.locator(".msg-s-message-group__name").textContent({ timeout: 5000 }).catch(() => senderName);
              const localTimestamp = await replyElement.locator(".msg-s-message-group__timestamp").textContent({ timeout: 5000 }).catch(() => timestamp);
              //console.log(`  - Message ${i + 1}: From ${localSender} at ${localTimestamp} - "${messageText.trim() || "No readable message content"}"`);
              console.log(
